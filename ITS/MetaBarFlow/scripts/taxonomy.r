...
for (i in 1:nrow(summary)) {
  summary[i,]$upper_margin<-summary[i,]$pident.max-summary[i,]$pident.min.best}  #Emil: changed to this loop to calculate for all rows
...
# Creat new column for pident.min.best in IDtable
IDtable$pident.min.best <- IDtable$pident

#Add pident.min.best to IDtable
for (i in unique (IDtable$qseqid)){
   IDtable[IDtable$qseqid==i,]$pident.min.best<-summary.best[summary.best$qseqid==i,]$pident.min.best
}
